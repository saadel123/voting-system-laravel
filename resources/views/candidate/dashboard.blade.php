                  ...
                  <div class="flex">
                      ...
                      <!-- Navigation Links -->
                            @if (Auth::user()->role == "candidate")
                               @if ($nominee_status == 1)
                                 <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                                   <x-nav-link :href="route('compaign')" :active="request()->routeIs('compaign')">
                                    {{ __('Run Compaigns') }}
                                   </x-nav-link>
                                 </div>
                               @endif
                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    {{-- alert for success ends --}}
                        <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            @if (Auth::user()->role == "candidate")
                              @if ($nominee_status == 0)
                                 Your approval for candidate is pending !
                              @endif
                             @endif

                             @if (Auth::user()->role == "candidate")
                               @if ($nominee_status == 1)
                                 Your are approved by admin to be a candidate ! <br>
                                 Now You can ran your compaign section !
                               @endif
                             @endif
                        </div>
                    </div>
